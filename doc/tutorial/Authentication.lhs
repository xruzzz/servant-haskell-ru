...
и тонкие темы; поскольку servant начал исследовать это пространство, мы начали
с одной из самых ранних схем аутентификации HTTP: [Базовая аутентификация](https://en.wikipedia.org/wiki/Basic_access_authentication).

Servant `0.5` shipped with out-of-the-box support for базовой аутентификации.
Однако, мы признаем, что каждое веб-приложение является его собственной красивой снежинкой
и предлагает экспериментальную поддержку для обобщенной или специальной(ad-hoc) аутентификации.

В этом уроке мы построим два API. Один из них защищает определенные маршруты с помощью
Базов аутентификаци, а другой защищает одни и те же маршруты с помощью собственной
схемы аутентификации.

## Базовая аутентификация

When protecting endpoints с базовой аутентификацией, we need to specify two
items:

1. **realm** of authentication as per the базов аутентификаци spec.
2. Тип данных returned by the server after authentication is verified. This
...
`BasicAuth` combinator to protect them under a Базов аутентификаци scheme (the
...
different function to better capture the semantics of Базов аутентификаци:
...
-- Базов аутентификаци requires a Context Entry with the 'BasicAuthCheck' value
...
## Обобщенная аутентификация(Generalized Authentication

Иногда схема аутентификации вашего сервера не совсем соответствует
стандартам (или, возможно, servant не развернул поддержку этой новой (hasn't rolled-out support for that new), fancy
схемы аутентификации). Для такого сценария, servant `0.5` обеспечивает легкую и
простую экспериментальную поддержку для сворачивания(to roll) собственной аутентификации.

Почему экспериментальный? Мы долго работали над дизайном для аутентификации. Мы
действительно изо всех сил пытались найти хороший, типо-безопасную нишу в пространстве дизайна(type-safe niche in the design space). По факту,
`Context` вышел из этой работы(came out of this work), и пока это действительно подходит для схем вроде Basic
и JWT, это было не достаточно, чтобы полностью поддерживать что-то вроде OAuth или HMAC, which
have flows, roles, and other fancy ceremonies. Далее, мы не были уверены *как*
люди будут использовать auth.

Так, в типичном модном запуске(in typical startup fashion), мы разработали MVP 'обобщенного(generalized) auth' и
выпустил его в экспериментальном модуле, надеясь получить от вас обратную связь!(released it in an experimental module, with the hope of getting feedback from you!
Итак, если вы читаете это или используете обобщенную поддержку auth, пожалуйста, сообщите нам свои отзывы!(So, if you're reading this or using generalized auth support, please give us
your feedback!)

### Что такое обобщенная аутентификация?

**TL;DR**: вы бросаете помеченный(throw a tagged) комбинатор `AuthProtect` в front of the endpoints
вы хотите защитить, а затем supply(предоставить, снабдить) функцию `Request -> Handler user`
которую мы запускаем в любое время, когда запрос соответствует защищенной конечной точке. Это точно решает
"Мне просто нужно защитить эти конечные точки функцией, которая выполняет некоторую
сложную бизнес-логику" и ничего более. За кулисами мы используем тип
family instance (`AuthServerData`) и `Context` чтобы достигнуть этого.

### Обобщенная аутентификация в дествии(Action)

Давайте реализуем тривиальную схему аутентификации. Мы будем защищать наш API
путем поиска файла cookie с именем `"servant-auth-cookie"`. Это значение cookie будет
содержать ключ, из которого мы можем найти `User`.
...
Для обобщенной аутентификации, servant exposes the `AuthHandler` type,
...
re-use the endpoints from our Базов аутентификаци example.
...
Создание обобщенной схемы ad-hoc-аутентификации было достаточно простым:

1. используйте комбинатор `AuthProtect` чтобы защитить ваш API.
2. выберите тип данных приложения, используемый вашим сервером, когда
аутентификация выполнена успешно (in our case this was `User`).
...
1. Модуль `Servant.Server.Experimental.Auth` содержит an orphan `HasServer`
instance for the `AuthProtect` combinator. You may be get orphan instance
warnings when using this.
2. Обобщенная аутентификация требует расширения `UndecidableInstances` extension.

## Аутентификация на стороне клиента(Client-side Authentication)

### Базова аутентификация

As of `0.5`, *servant-client* поставляется с поддержкой для базовой аутентификации!
Endpoints protected by Базов аутентификаци will require a value of type
`BasicAuthData` to complete the request.

### Обобщенная аутентификация(Generalized Authentication)

Servant `0.5` также поставляется с поддержкой обобщенной аутентификации. Similar
