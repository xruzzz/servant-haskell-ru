# Опрашивая(Querying) API

Определяя обработчики, которые(While defining handlers that) [обслуживают(serve an) API](Server.lhs) has a lot to it, querying an API is simpler: we do not care about what happens inside the webserver, we just need to know how to talk to it and get a response back. That said, we usually have to write the querying functions by hand because the structure of the API isn't a first class citizen and can't be inspected to generate the client-side functions.

**servant** однако имеет возможность проверить API-ы, потому что API-ы являются просто типами Haskell и (GHC) Haskell позволяет нам делать несколько вещей с типами. In the same way that we look at an API type to deduce the types the handlers should have, we can inspect the structure of the API to *derive* Haskell functions that take one argument for each occurrence of `Capture`, `ReqBody`, `QueryParam`
...
Исходником(source) для этой секции руководства является грамотный(literate) haskell файл, так во-первых мы нуждаемся в том, чтобы иметь некоторые расширения языка и импорт:
...
Также, мы нуждаемся в примерах for some domain specific data types:
