# Генерирование  функций Javascript для запроса API

Теперь мы посмотрим, как **servant** позволяет вам превратить тип API в функции javascript
that you can call to query a webservice.

Для этого, мы рассмотрим простую страницу, разделенную на две части. At the top, we
...
### Генератор сервиса(Service generator)

Вы также можете автоматически генерировать сервис, чтобы обернуть весь API как
единый Угловой сервис(a single Angular service):
...
# Пользовательский конструктор имен функций(Custom function name builder)
